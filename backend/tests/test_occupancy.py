...
def test_minimum_observations_rule(db_session):
    unique_code = f"TEST-T-OCC-{uuid.uuid4().hex[:6]}"
    t = Tiger(tiger_code=unique_code, callsign="Test Transient", status="transient", primary_zone="Core")
    db_session.add(t)
    db_session.flush()

    st = db_session.query(CameraStation).first()
    if not st:
        st = CameraStation(code=f"ST-{uuid.uuid4().hex[:4]}", name="Test Station", latitude=21.75, longitude=79.32, zone="core", range_beat="Turia", habitat="Sal")
        db_session.add(st)
        db_session.flush()

    now = datetime.now(timezone.utc)
    img1 = Image(filename="occ_01.jpg", original_path="data/raw/occ_01.jpg", storage_path="data/images/occ_01.jpg", file_hash=uuid.uuid4().hex, station_id=st.id, captured_at=now)
    img2 = Image(filename="occ_02.jpg", original_path="data/raw/occ_02.jpg", storage_path="data/images/occ_02.jpg", file_hash=uuid.uuid4().hex, station_id=st.id, captured_at=now)
